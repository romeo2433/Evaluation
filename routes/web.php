...
Route::group(['prefix' => 'reinitialisation'], function () {
    Route::get('/', [DropController::class, 'showReinitialisation'])->name('reinitialisation.index'); // Correction ici
    Route::post('/truncate', [DropController::class, 'truncateTable'])->name('truncate');
});


Route::group(['prefix' => 'data'], function () {
    Route::get('/cleaning', 'DataManagementController@cleaning')->name('data.cleaning');
    Route::get('/import', 'DataManagementController@import')->name('data.import');
    Route::get('/generation', 'DataManagementController@generation')->name('data.generation');
    Route::post('/clean-tables', 'DataManagementController@cleanTables')->name('data.clean-tables');
    Route::post('/import-data', 'DataManagementController@importData')->name('data.import-data');
    Route::post('/generate-data', 'DataManagementController@generateData')->name('data.generate-data');
});
